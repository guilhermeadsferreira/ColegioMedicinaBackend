...
class AddressController {
  async store(req, res) {
    try {
      await _Address2.default.create(req.body);
  ...
  async delete(req, res) {
    try {
      await _Address2.default.remove({ _id: req.params.id });
      return res.status(200).json(_handle_error.handle_response.call(void 0, "success", ""));
    } catch (err) {
      return res.status(500).json(_handle_error.handle_response.call(void 0, "error"));
    }
  ...
  async findAll(req, res) {
    try {
      const data = await _Address2.default.find({
  ...
  async update(req, res) {
    try {
      await _Address2.default.updateOne({ _id: req.body.id }, req.body);
      const data = await _Address2.default.findById(req.body.id);
      return res.status(200).json(_handle_error.handle_response.call(void 0, "success", "", data));
    } catch (err) {
      return res.status(500).json(_handle_error.handle_response.call(void 0, "error"));
    }
